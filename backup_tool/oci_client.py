...
class OCIObjectStorageClient():
    ...
    def __init__(self, logger=None, instance_principal=False, **kwargs):
        ...
        oci_bucket_name     :   Name of oci bucket
    ...
    def object_list(self):
        ...
        self.logger.info("Retrieving object list from namespace %s and bucket %s",
                         self.namespace_name, self.bucket_name)
        all_objects_response = list_call_get_all_results(self.object_storage_client.list_objects,
                                                         self.namespace_name, self.bucket_name, fields='name,md5,size,timeCreated')
        return [to_dict(obj) for obj in all_objects_response.data.objects]


    def object_put(self, object_name, file_name, md5_sum=None, resume_upload=False):
        '''
        ...
        '''
        self.logger.info(f'Starting upload of file "{file_name}" to namespace "{self.namespace_name}" '
                         f'bucket "{self.bucket_name}" and object name "{object_name}"')
        ...
        if resume_upload:
            ...
            for multipart_upload in multipart_uploads.data:
                ...
                if multipart_upload.object == object_name:
                    ...
                    response = self.upload_manager.resume_upload_file(self.namespace_name, self.bucket_name, object_name, file_name, multipart_upload.upload_id)
                    upload_resumed = True
                    break
        if not upload_resumed:
            response = self.upload_manager.upload_file(self.namespace_name, self.bucket_name, object_name, file_name, content_md5=md5_sum)
    ...
    def object_get(self, object_name, file_name, set_restore=False):
        '''
        ...
        '''
        self.logger.info(f'Downloading object "{object_name}" from namespace "{self.namespace_name}" and bucket "{self.bucket_name}" to file "{file_name}"')
        with open(file_name, 'wb') as writer:
            try:
                get_response = self.object_storage_client.get_object(self.namespace_name,
                                                                     self.bucket_name,
                                                                     object_name)
                ...
                if set_restore and "'code': 'NotRestored'" in str(error):
                    self.logger.debug(f'Object "{object_name}" in bucket "{self.bucket_name}" and namepsace '
                                      f'"{self.namespace_name}" is archived, will mark for restore')
                    restore_details = RestoreObjectsDetails(object_name=object_name)
                    restore_response = self.object_storage_client.restore_objects(self.namespace_name, self.bucket_name, restore_details)
                    ...
                    self.logger.info(f'Set restore on object "{object_name}" in bucket "{self.bucket_name}" and namespace "{self.namespace_name}"')
                return False
    ...
    def object_delete(self, object_name):
        ...
        '''
        self.logger.info(f'Deleting object "{object_name}" from namespace "{self.namespace_name}" and bucket "{self.bucket_name}"')
        response = self.object_storage_client.delete_object(self.namespace_name,
                                                            self.bucket_name,
                                                            object_name)
