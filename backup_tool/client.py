...
class BackupClient():
    ...
    def _file_backup_ensure_database_entry(self, local_file_path, local_file_md5, overwrite, check_uploaded_md5):
        ...
        if local_backup_file:
            return self._file_backup_file_exists(local_backup_file, local_file_path,
                                                 local_file_md5, overwrite, check_uploaded_md5), local_backup_file
    ...
    def _file_backup_encrypt(self, local_file_path, local_file_md5):
        with utils.temp_file(self.work_directory, delete=False) as encrypted_file:
            ...
            offset, check_local_file_md5, encrypted_file_md5 = crypto.encrypt_file(str(local_file_path), str(encrypted_file), self.crypto_key)
    ...
    def _file_backup_upload(self, encrypted_file, local_encrypted_file_md5, offset, local_backup_file, object_path=None, resume_upload=False):
        # Check if md5 file already exists
        backup_entry = self.db_session.query(BackupEntry).\
                filter(BackupEntry.uploaded_md5_checksum == local_encrypted_file_md5).first()
        ...
        if backup_entry:
            ...
            self.logger.debug(f'Found existing upload with matching md5 found {backup_entry.id} for '
                              f'file with md5 "{local_encrypted_file_md5}"')
        ...
        self.logger.info(f'No encrypted upload matching file with md5 {local_encrypted_file_md5}, uploading copy')
    ...
    def file_backup(self, local_file, overwrite=False, check_uploaded_md5=False):
        ...
        should_upload_file, local_backup_file = self._file_backup_ensure_database_entry(local_file, local_file_md5, overwrite, check_uploaded_md5)
